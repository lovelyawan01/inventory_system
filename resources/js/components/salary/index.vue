 <template>
  ...
  <div class="row">
    <router-link to="/salary" class="btn  btn-primary">Add Salary</router-link>
  </div>
                  ...
                  <table class="table align-items-center table-flush">
                    ...
                    <tbody>
                      <tr v-for="salary in filtersearch" :key="salary.id" class="mt-r">
                        <td> {{ salary.salary_month.toUpperCase() }} </td>
                        <td>
    ...
    deletesalary(id){
      Swal.fire({
        ...
        if (result.isConfirmed) {
          axios.delete('/api/sallary/'+id)
          .then(() => {
            this.salary = this.salary.filter(salary => {
              return salary.id != id
            })
          })
          .catch(() => {
